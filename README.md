...
Dự án được chia thành các thư mục và tệp nhỏ để dễ dàng quản lý, chỉnh sửa, tránh việc dồn toàn bộ code vào một tệp duy nhất.
...
📦 Source_code_bien_soan_TL_dai
 ...
 ┣ 📂 NganHang/              # Thư mục chứa các file ngân hàng câu hỏi dùng để bốc ngẫu nhiên.
 ┃ ┗ 📜 bank.tex           # Ví dụ một ngân hàng câu hỏi.
...
2. Mở file `main.tex`.
3. Bấm **Ctrl + Alt + B** (hoặc nút Build màu xanh góc phải trên cùng).
...
*Lưu ý:* Việc đẩy các tệp tạm (`.aux`, `.log`...) vào thư mục `build/` giúp thư mục gốc của bạn luôn sạch sẽ, gọn gàng, không bị sinh "rác". Tuyệt đối không tự bấm chạy terminal lệnh `xelatex main.tex` mà không chỉ định thư mục build, vì nó sẽ xả rác ra ngoài gốc.
...
### 2. Chia Phần (Part)
Dùng để chia đề thi thành nhiều phần (Trắc nghiệm khách quan, Trắc nghiệm đúng sai, Trả lời ngắn).
...
### 4. Bốc Câu Hỏi Ngẫu Nhiên Từ Ngân Hàng (TÍNH NĂNG MỚI)
Thay vì tự tay gõ từng câu, bạn có thể tạo một file ngân hàng (ví dụ `NganHang/bank.tex`) chỉ chứa danh sách các lệnh `\caugiai` liên tiếp nhau. Sau đó, ở file `main.tex`, bạn gọi các lệnh sau để bốc số câu tuỳ ý:

```tex
% Đặt hạt giống để đề không bị đổi mỗi lần biên dịch lại. Đổi số khác để có đề mới.
\DatHatGiongNgauNhien{123} 

% Nạp toàn bộ ngân hàng vào bộ nhớ
\LoadNganHang{NganHang/bank.tex}

% Rút ngẫu nhiên 3 câu để in ra
\LayCauHoiNgauNhien{3}

% Dọn dẹp bộ nhớ nếu muốn load file ngân hàng khác ở phần tiếp theo (không bắt buộc)
\XoaNganHang
```
...
Tại tệp `Preambles/Macros.tex`, bạn có thể kiểm soát việc in ra lời giải hay in ra đề bài trống. Tìm tới đoạn `CHE DO GIAO VIEN / HOC SINH`:
- Dùng `\giaovientrue` (Bật hiển thị Lời giải, in đáp án chi tiết ở cuối).
- Dùng `\giaovienfalse` (Tắt hiển thị Lời giải, in bản trắng cho học sinh).
